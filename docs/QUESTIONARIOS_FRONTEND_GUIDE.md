...
### 7.3 Verificar se cliente já respondeu (público)

```
GET /api/v1/public/{slug}/questionarios/{id}/respostas/verificar?clienteId=12
```

Equivalente público do `6.5`. Retorna `{ "respondido": true|false }`.

Use **antes** de exibir o formulário ao cliente (evita 400 ao submeter).
...
1. Antes de exibir, chamar `GET /verificar?clienteId=...` (ou `verificar-agendamento`) — se já respondeu, mostrar tela de confirmação em vez do form. **No fluxo público (sem JWT)**, use os endpoints da seção 7 (`/api/v1/public/{slug}/...`).
